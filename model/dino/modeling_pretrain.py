...
class iTPNForMIM(Backbone):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2, init_values=0.1, attn_head_dim=None,
    ...
    def interpolate_pos_encoding(self, x, h, w):
        ...
        patch_pos_embed = self.pos_embed.clone()
        dim = x.shape[-1]
    ...
    def forward_features(self, x, bool_masked_pos=None):
        ...
        # print(B,C,H,W)
        # print('---------------------------------------')
        x = self.patch_embed(x, bool_masked_pos=bool_masked_pos, mask_token=None)  # B*L*4*4*C
    ...
    def forward(self, x_all):
        x = x_all.tensors
        features = self.forward_features(x,bool_masked_pos=None)
