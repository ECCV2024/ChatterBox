...
from .segmentation import (DETRsegm, PostProcessPanoptic, PostProcessSegm,
                           dice_loss)
from .deformable_transformer_TgtQueries import build_deformable_transformer
...
class DINO(nn.Module):
    ...
    def forward_enc_dec(self, samples, features, poss, targets, tgt_query=None, refpoint_query=None):
        ...
        if False:  # self.dn_number > 0 or targets is not None:
            input_query_label, input_query_bbox, attn_mask, dn_meta = \
                prepare_for_cdn(dn_args=(targets, self.dn_number, self.dn_label_noise_ratio, self.dn_box_noise_scale),
                                training=self.training, num_queries=self.num_queries, num_classes=self.num_classes,
                                hidden_dim=self.hidden_dim, label_enc=self.label_enc)
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs, masks, input_query_bbox, poss,
                                                                             input_query_label, attn_mask, tgt_query=tgt_query, refpoint_query=refpoint_query)
    ...
    def forward(self, samples: NestedTensor, targets: List = None, tgt_query=None, refpoint_query=None):
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='dino_v2_TgtQueries')
def build_dino(args):
