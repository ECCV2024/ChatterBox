...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 21841, 'input_size': (3, 224, 224), 'pool_size': None,
...
class ConvSwiGLU(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x1 = self.w1(x).flatten(2).transpose(1, 2).contiguous()
        x2 = self.w2(x).flatten(2).transpose(1, 2).contiguous()
        hidden = self.act(x1) * x2
        x = self.ffn_ln(hidden).transpose(1, 2).view(B, C, H, W).contiguous()
        x = self.w3(x)
        x = self.drop(x)
        return x
...
class Attention(nn.Module):
    def __init__(
            ...
            attn_head_dim=None, use_decoupled_rel_pos_bias=False, deepnorm=False, subln=False, norm_layer=nn.LayerNorm,
            xattn=False, rope=None,
    ):
        super().__init__()
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        ...
        if self.deepnorm or self.subln:
            ...
            qkv = qkv.reshape(B, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4).contiguous()  # 3, B, num_heads, N, C
    ...
    def forward(self, x):
        ...
        if self.gamma_2 is None:
            ...
            if self.postnorm:
                ...
                m = self.mlp(self.norm2(x.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous())
                #print('------------------------- m:', m.shape)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        ).permute(0, 2, 4, 3, 5, 1).contiguous().reshape(B, num_patches, self.inner_patches, self.inner_patches, -1).contiguous()
...
class ConvPatchSplit(nn.Module):
    def __init__(self, dim, fpn_dim, norm_layer):
        super().__init__()
        self.norm = norm_layer(dim)
        self.reduction = torch.nn.ConvTranspose2d(dim, fpn_dim, kernel_size=2, stride=2, padding=0)
        self.fpn_dim = fpn_dim
...
class iTPN(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=False, use_checkpoint=True, num_classes=1000, use_mean_pooling=False,
                 init_std=0.02,
                 init_scale=0.01,
                 cls_token=False,
                 grad_ckpt=True,
        ...
        #     mlvl_dims=mlvl_dims
        # elif out_num==4:
        #     mlvl_dims .update({'32':embed_dim*2})
        mlvl_dims = {'4': embed_dim // 4, '8': embed_dim // 2, '16': embed_dim,'32':embed_dim*2}
        ...
        ##
        # self._out_features = ['s1', 's2', 's3','s4']
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks[:-self.num_main_blocks]:
            if isinstance(blk, PatchMerge) :
                ...
                outs.append(out)
            elif isinstance(blk, ConvPatchMerge):
                outs.append(x)
        ...
        outs[0] = outs[0].reshape(B, -1, Hp * 4, Wp * 4)
        outs[1] = outs[1].reshape(B, -1, Hp * 2, Wp * 2)
        outs[2] = outs[2].transpose(1, 2).reshape(B, -1, Hp, Wp)
        print('backbone_self.forward_features(x)_outs[0].grad_fn:{}'.format(outs[0].grad_fn))
        print('backbone_self.forward_features(x)_outs[1].grad_fn:{}'.format(outs[2].grad_fn))
        print('backbone_self.forward_features(x)_outs[2].grad_fn:{}'.format(outs[2].grad_fn))

        if len(self.mlvl_dims) == 4:
            out3=self.downsample(outs[2])
            print('backbone_self.forward_features(x)_outs[3].grad_fn:{}'.format(out3.grad_fn))
            outs.append(out3)
    ...
    def forward(self, x_all):
        ...
        #mlvl_dims = {'4': embed_dim // 4, '8': embed_dim // 2, '16': embed_dim, '32': embed_dim * 2}
        for idx, out_i in enumerate(self.mlvl_dims):
            m = x_all.mask
...
def itpn_base_fusedLN_NaiveSwiGLU_subln_xavier_3324_patch16_224(pretrained=False, **kwargs):
    model = iTPN(
       patch_size=16, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24, num_heads=8, bridge_mlp_ratio=3.,
       mlp_ratio=4 * 2 / 3, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
       xattn=False,
       convmlp=True,
       naiveswiglu=True,
       subln=True,
...
def build_itpn_base_fusedLN_NaiveSwiGLU_subln_xavier_3324_patch16_224_transformer(modelname,pretrained=False,**kwargs):
    assert modelname in ['iTPN_base']
    ...
    model_para_dict = {
        'iTPN_base':dict(
            ...
            #in_chans=3,
            embed_dim=512,
            depth_stage1=3, depth_stage2=3, depth=24, num_heads=8, bridge_mlp_ratio=3.,
            mlp_ratio=4 * 2 / 3, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
            xattn=False,
            convmlp=True,
            naiveswiglu=True,
