...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        #self.compress_text=nn.Linear(dim_text,d_model)
        #self.merge_text_tgt=MLP(d_model,d_model)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):#,sparse_embedding=None,label_enc=None,dn_labelbook_size=None,pn_idx=None
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            #print('for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds))')
            #print('src.dtype:{}'.format(src.dtype))
            #print('mask.dtype:{}'.format(mask.dtype))
            #print('pos_embed.dtype:{}'.format(pos_embed.dtype))
            bs, c, h, w = src.shape
            spatial_shape = (h, w)
        ...
        ########################################################
        # assert tgt is None
        # tgt=self.compress_text(sparse_embedding)#bs 1 d->bs 1 d_model
        # if label_enc is None:
        #     tgt=tgt.repeat(1,refpoint_embed.shape[1],1)#bs 1 d_model->bs num_dn d_model
        # else:
        #     assert dn_labelbook_size is not None
        #     false_label=dn_labelbook_size+1#no class
        #     false_label_embed = label_enc(false_label)
        #     tgt=tgt.comca
        ########################################################
        if self.two_stage_type == 'standard':
            ...
            if refpoint_embed is not None:
                ...
                ###############################
                #print('tgt_before_cat.shape :{}'.format(tgt.shape))
                tgt = torch.cat([tgt, tgt_], dim=1)#tgt:dn_gt, tgt_:from encoder
                #print('tgt_after_cat.shape :{}'.format(tgt.shape))
            else:
        ...
        # Begin Decoder
        #########################################################
        # tgt: bs num_gt_dn+num_from_encoder d_model
        # add_text:
...
class TransformerEncoder(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ref_y, ref_x = torch.meshgrid(torch.linspace(0.5, H_ - 0.5, H_, dtype=torch.float16, device=device),
                                          torch.linspace(0.5, W_ - 0.5, W_, dtype=torch.float16, device=device))
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        #for itm_out in intermediate:
            #print("itm_out.dtype:{}".format(itm_out.dtype))
        #for itm_refpoint in ref_points :
            #print("itm_refpoint.dtype:{}".format(itm_refpoint.dtype))
...
def build_deformable_transformer(args):
    ...
    return DeformableTransformer(
        ...
        num_unicoder_layers=args.unic_layers,  # what is it
        num_decoder_layers=args.dec_layers,
        normalize_before=args.pre_norm,
        return_intermediate_dec=True,
        query_dim=args.query_dim,
        activation=args.transformer_activation,
        num_patterns=args.num_patterns,  # what is it
        modulate_hw_attn=True,

        deformable_encoder=True,
        deformable_decoder=True,
        num_feature_levels=args.num_feature_levels,
        enc_n_points=args.enc_n_points,  # what is it
        dec_n_points=args.dec_n_points,  # what is it
        use_deformable_box_attn=args.use_deformable_box_attn,  # what is it
        box_attn_type=args.box_attn_type,  # what is it

        learnable_tgt_init=True,
        decoder_query_perturber=decoder_query_perturber,  # what is it
