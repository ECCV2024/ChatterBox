...
# from .hivit_eva02 import build_hivit_eva02_transformer
#from .modeling_finetune import build_itpn_base_fusedLN_NaiveSwiGLU_subln_xavier_3324_patch16_224_transformer
from .modeling_finetune_debug import build_itpn_base_fusedLN_NaiveSwiGLU_subln_xavier_3324_patch16_224_transformer
from .position_encoding import build_position_encoding
from .convnext import build_convnext
from .swin_transformer import build_swin_transformer
from .modeling_pretrain import itpn_base_3324_patch16_224
...
def build_backbone(args):
    ...
    if args.backbone in ['resnet50', 'resnet101']:
    ...
    elif args.backbone in ['hivit_eva02']:
        backbone = build_hivit_eva02_transformer(args.backbone)
        ...
        bb_num_channels = backbone.num_features[0:len(return_interm_indices)]
    elif args.backbone in ['iTPN_base']:
        backbone = build_itpn_base_fusedLN_NaiveSwiGLU_subln_xavier_3324_patch16_224_transformer(args.backbone)
        ...
        if "backbone_dir" in args:
            ...
            print(str(_tmp_st_output))
        bb_num_channels = backbone.num_features[0:len(return_interm_indices)]
    elif args.backbone in ['Fast_iTPN_base']:
        backbone = itpn_base_3324_patch16_224(args.backbone)
        #return_interm_indices = [0, 1, 2,3]
        if "backbone_dir" in args:
            pretrained_dir = args.backbone_dir
            checkpoint = torch.load(pretrained_dir, map_location='cpu')['module']
            from collections import OrderedDict
            _tmp_st = OrderedDict({k: v for k, v in clean_state_dict(checkpoint).items()})
            _tmp_st_output = backbone.load_state_dict(_tmp_st, strict=False)
            print(str(_tmp_st_output))
