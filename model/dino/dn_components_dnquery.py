...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, hidden_dim, label_enc, sparse_embedding=None, compress_text=None):
    ...
    if training:
        ...
        padding_label = torch.zeros(pad_size, hidden_dim, dtype=input_label_embed.dtype).cuda()
        padding_bbox = torch.zeros(pad_size, 4, dtype=input_label_embed.dtype).cuda()
        ...
        if len(known_bid):
            input_query_label = input_query_label.to(input_label_embed.dtype)
