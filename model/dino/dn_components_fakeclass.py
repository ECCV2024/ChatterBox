...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, hidden_dim, label_enc, sparse_embedding=None, compress_text=None):
    ...
    if training:
        ...
        if label_noise_ratio > 0:
            ...
            # #unknown_label_embedding=unknown_label_embedding.half()
        ...
        positive_idx = torch.tensor(range(len(boxes)),dtype=torch.float16).long().cuda().unsqueeze(0).repeat(dn_number, 1)
        positive_idx += (torch.tensor(range(dn_number),dtype=torch.float16) * len(boxes) * 2).long().cuda().unsqueeze(1)
        positive_idx = positive_idx.flatten()
        negative_idx = positive_idx + len(boxes)
        if box_noise_scale > 0:
            known_bbox_ = torch.zeros_like(known_bboxs,dtype=torch.float16)
            ...
            rand_sign = torch.randint_like(known_bboxs, low=0, high=2, dtype=torch.float16) * 2.0 - 1.0
            ...
            # print('known_bbox_expand.dtype:{}'.format(known_bbox_expand.dtype))

        m = known_labels_expaned.long().to('cuda')
        input_label_embed = label_enc(m)
        input_bbox_embed = inverse_sigmoid(known_bbox_expand,half=1)

        padding_label = torch.zeros(pad_size, hidden_dim, dtype=torch.float16).cuda()
        padding_bbox = torch.zeros(pad_size, 4, dtype=torch.float16).cuda()
